        ...
        {page==="patient" && (()=>{
          ...
          return (
            <div style={{maxWidth:900,margin:"0 auto"}}>
              {/* Hero */}
              <div style={{background:"linear-gradient(135deg,#0a1628,#0f2040)",border:`1px solid #1e3a5f`,borderRadius:16,padding:"32px 36px",marginBottom:24,position:"relative",overflow:"hidden"}}>
                <div style={{position:"absolute",right:-20,top:-20,width:160,height:160,borderRadius:"50%",background:"#1d4ed8",opacity:0.06}} />
                <div style={{fontSize:10,color:"#1e3a5f",letterSpacing:"0.18em",textTransform:"uppercase",marginBottom:8}}>Surgeon Outcomes · General Surgery</div>
                <div style={{fontSize:28,fontWeight:700,color:"#f1f5f9",letterSpacing:"-0.02em",marginBottom:10}}>{settings.surgeonName||"Surgical Outcomes"}</div>
                {sv.introText&&<div style={{fontSize:13,color:"#334155",maxWidth:620,lineHeight:1.7,marginBottom:16}}>{sv.introText}</div>}
                <div style={{display:"flex",gap:28}}>
                  {[[cases.length,"Cases Performed"],[tplsWithCases.length,"Procedure Types"],["ACS","Member"]].map(([v,l])=>(
                    <div key={l}><div style={{fontSize:10,color:"#1e3a5f",textTransform:"uppercase",letterSpacing:"0.12em"}}>{l}</div><div style={{fontSize:22,fontWeight:700,color:"#e2e8f0",fontFamily:"'JetBrains Mono',monospace"}}>{v}</div></div>
                  ))}
                </div>
              </div>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:20}}>
                <button onClick={()=>setSelTpl(null)} style={{padding:"8px 16px",borderRadius:8,border:`1px solid ${!selTpl?"#1d4ed8":"#1e293b"}`,background:!selTpl?"#1e3a5f":"transparent",color:!selTpl?"#93c5fd":"#475569",fontSize:13,fontWeight:!selTpl?600:400}}>All Procedures</button>
                {tplsWithCases.map(t=>(
                  <button key={t.id} onClick={()=>setSelTpl(t.id)} style={{padding:"8px 16px",borderRadius:8,border:`1px solid ${selTpl===t.id?t.color:"#1e293b"}`,background:selTpl===t.id?t.color+"22":"transparent",color:selTpl===t.id?t.color:"#475569",fontSize:13,fontWeight:selTpl===t.id?600:400}}>
                    {t.emoji} {t.patientName||t.name}
                  </button>
                ))}
              </div>

              {tplObj?.patientDesc&&<div style={{fontSize:13,color:"#475569",lineHeight:1.7,marginBottom:20,padding:"14px 18px",background:"#0a1220",borderRadius:10,border:"1px solid #131f30"}}>{tplObj.patientDesc}</div>}

              {/* KPIs */}
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
                {sv.showVolume&&<div style={{background:"#0a1220",border:"1px solid #131f30",borderRadius:12,padding:"18px 22px"}}><div style={{fontSize:10,color:"#334155",textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:6}}>Cases Performed</div><div style={{fontSize:32,fontWeight:700,color:"#f1f5f9",fontFamily:"'JetBrains Mono',monospace"}}>{s.n}</div><div style={{fontSize:11,color:"#1e3a5f",marginTop:4}}>{selTpl?`${tplObj?.patientName||tplObj?.name} procedures`:"across all procedure types"}</div></div>}
                {sv.showComplications&&<div style={{background:"#0a1220",border:"1px solid #131f30",borderRadius:12,padding:"18px 22px"}}><div style={{fontSize:10,color:"#334155",textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:6}}>Complication Rate</div><div style={{fontSize:32,fontWeight:700,color:s.compPct>10?"#f59e0b":"#10b981",fontFamily:"'JetBrains Mono',monospace"}}>{pct(s.comps.length,s.n)}%</div><div style={{fontSize:11,color:"#1e3a5f",marginTop:4}}>Any complication requiring attention</div></div>}
                {sv.showReadmission&&<div style={{background:"#0a1220",border:"1px solid #131f30",borderRadius:12,padding:"18px 22px"}}><div style={{fontSize:10,color:"#334155",textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:6}}>30-Day Readmission</div><div style={{fontSize:32,fontWeight:700,color:s.readmitPct>8?"#f97316":"#10b981",fontFamily:"'JetBrains Mono',monospace"}}>{pct(s.readmits.length,s.n)}%</div><div style={{fontSize:11,color:"#1e3a5f",marginTop:4}}>Unplanned return to hospital</div></div>}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
                {sv.showLOS&&s.avgLOS>0&&<div style={{background:"#0a1220",border:"1px solid #131f30",borderRadius:12,padding:"18px 22px"}}><div style={{fontSize:10,color:"#334155",textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:6}}>Average Hospital Stay</div><div style={{fontSize:28,fontWeight:700,color:"#f1f5f9",fontFamily:"'JetBrains Mono',monospace"}}>{s.avgLOS.toFixed(1)} days</div></div>}
                {sv.showConversion&&<div style={{background:"#0a1220",border:"1px solid #131f30",borderRadius:12,padding:"18px 22px"}}><div style={{fontSize:10,color:"#334155",textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:6}}>Minimally Invasive Rate</div><div style={{fontSize:28,fontWeight:700,color:"#38bdf8",fontFamily:"'JetBrains Mono',monospace"}}>{pct(s.mis.length,s.n)}%</div><div style={{fontSize:11,color:"#1e3a5f",marginTop:4}}>Laparoscopic or robotic approach</div></div>}
                {sv.showCost&&s.avgCost>0&&<div style={{background:"#0a1220",border:"1px solid #131f30",borderRadius:12,padding:"18px 22px"}}><div style={{fontSize:10,color:"#334155",textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:6}}>Average Case Cost</div><div style={{fontSize:28,fontWeight:700,color:"#4ade80",fontFamily:"'JetBrains Mono',monospace"}}>{dollar(Math.round(s.avgCost))}</div><div style={{fontSize:11,color:"#1e3a5f",marginTop:4}}>Facility-level cost data</div></div>}
              </div>

              {/* Satisfaction */}
              {sv.showSatisfaction&&totalSat>0&&(
                <div style={{background:"#0a1220",border:"1px solid #131f30",borderRadius:12,padding:"18px 22px",marginBottom:20}}>
                  <div style={{fontSize:13,fontWeight:600,color:"#e2e8f0",marginBottom:14}}>Patient Satisfaction (from follow-up data)</div>
                  <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                    ...
                    <div style={{fontSize:11,color:"#334155",alignSelf:"center"}}>Based on {totalSat} follow-up responses</div>
                  </div>
                </div>
              )}
              ...
              {!selTpl&&sv.showVolume&&(
                <div style={{background:"#0a1220",border:"1px solid #131f30",borderRadius:12,padding:"18px 22px",marginBottom:20}}>
                  <div style={{fontSize:13,fontWeight:600,color:"#e2e8f0",marginBottom:14}}>Experience by Procedure Type</div>
                  {tplsWithCases.map(t=>{
                    const cnt=cases.filter(c=>c.templateId===t.id).length;
                    return (
                      <div key={t.id} style={{marginBottom:10}}>
                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                          <span style={{fontSize:12.5,color:"#94a3b8"}}>{t.emoji} {t.patientName||t.name}</span>
                          <span style={{fontSize:12,fontFamily:"'JetBrains Mono',monospace",color:t.color}}>{cnt} cases</span>
                        </div>
                        <div style={{height:8,background:"#0f1a2a",borderRadius:4}}><div style={{width:`${(cnt/cases.length)*100}%`,height:"100%",background:t.color,borderRadius:4,opacity:0.8}} /></div>
                      </div>
                    );
                  })}
                </div>
              ...
              <div style={{background:"#0a1220",border:"1px solid #131f30",borderRadius:10,padding:"14px 18px",fontSize:11,color:"#1e3a5f",lineHeight:1.7}}>
                <strong style={{color:"#334155"}}>About this data:</strong> All figures are self-reported and de-identified. Complication rates reflect my personal logged experience and should be understood in context of each patient's individual risk factors and case complexity. This information is provided to support informed consent and shared decision-making — not as a guarantee of outcome. For questions, please speak with me directly.
              </div>
            </div>
          );
        })()}
